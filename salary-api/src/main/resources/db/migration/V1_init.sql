...
BEGIN
    CREATE TABLE Employee (
        ...
        entry_date DATE,
        leave_date DATE,
        status TINYINT DEFAULT 1, -- 1:在职 2:离职 3:试用期
        create_time DATETIME DEFAULT GETDATE(),
        update_time DATETIME DEFAULT GETDATE()
    );
END
...
-- 创建工资配置表
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SalaryConfig]') AND type in (N'U'))
BEGIN
...
BEGIN
    CREATE TABLE Attendance (
        ...
        check_in DATETIME,
        check_out DATETIME,
        status TINYINT, -- 1:正常 2:迟到 3:早退 4:旷工 5:请假
        remark NVARCHAR(200),
        create_time DATETIME DEFAULT GETDATE()
    );
END
...
BEGIN
    CREATE TABLE Leave (
        ...
        emp_id INT FOREIGN KEY REFERENCES Employee(emp_id),
        leave_type TINYINT, -- 1:年假 2:病假 3:事假 4:婚假 5:产假 6:丧假
        start_time DATETIME,
        end_time DATETIME,
        duration DECIMAL(5,1), -- 请假天数
        reason NVARCHAR(200),
...
-- 创建加班记录表
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Overtime]') AND type in (N'U'))
BEGIN
...
BEGIN
    CREATE TABLE SysUser (
        ...
        emp_id INT FOREIGN KEY REFERENCES Employee(emp_id),
        role_type TINYINT, -- 1:超级管理员 2:管理员 3:人事 4:财务 5:普通用户
...
BEGIN
    ...
    VALUES ('系统管理部', '负责系统管理维护');
    
    INSERT INTO Employee (emp_no, emp_name, gender, position, dept_id, entry_date, status)
    VALUES ('ADMIN001', '系统管理员', 'M', '系统管理员', 1, GETDATE(), 1);
    
    -- 创建管理员账号（密码为: admin123）
    INSERT INTO SysUser (username, password, emp_id, role_type, status)
    VALUES ('admin', 'e10adc3949ba59abbe56e057f20f883e', 1, 1, 1);
END
GO